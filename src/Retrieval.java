...
public class Retrieval
{
	...
	{
		L1(new ManhattanDistance()), L2(new EuclideanDistance());
	...
	public static class DocumentSimilarity
	{
		...
		public DocumentSimilarity(double distance, String sourceDocument,
				String targetDocument, String index)
		{
	...
	@Argument(multiValued = true, required = true, index = 0, usage = "the names of the query documents", metaVar = "QUERY")
	private List<String> queryDocuments;
	@Option(name = "-k", required = false, usage = "the number k of to-be-retrieved documents")
	private int k = 5;
	@Option(name = "-m", aliases = { "--measure" }, required = false, usage = "the similarity function to be used for similarity retrieval")
	...
	@Option(name = "-q", aliases = { "--query" }, multiValued = true, required = false, usage = "the query words to be used")
	...
	public void run() throws Exception
	{
		...
		Multimap<String, DocumentSimilarity> similarities = HashMultimap
				.create();	
		...
		for (File indexFile : indices)
		{
			...
			// Determine class and document attribute -------
			...
			while (attributes.hasMoreElements())
			{
				...
				if (classAttribute == null
						&& attribute.name().matches(".*[Cc]lass.*"))
					classAttribute = attribute;

				if (documentAttribute == null
						&& attribute.name().matches(".*[Dd]ocument.*"))
					documentAttribute = attribute;

				if (documentAttribute != null && classAttribute != null)
					break;
			}

			if (classAttribute == null)
			{
				System.err.println("No class attribute found for index "
						+ indexFile);
			...
			if (documentAttribute == null)
			{
				System.err.println("No document attribute found for index "
						+ indexFile);
				System.err.println("Aborting");
				System.exit(1);
			}
			...
			while (instances.hasMoreElements())
			{
				...
				if (!queryDocuments.contains(document))
					continue;

				documentVectors.add(instance);
			}

			similarityMeasure.getDistanceFunction()
					.setInstances(indexInstances);
			...
			while (instances.hasMoreElements())
			{
				...
				for (Instance queryInstance : documentVectors)
				{
					...
					double distance = similarityMeasure.getDistanceFunction()
							.distance(queryInstance, instance);

					similarities.put(queryInstanceName, new DocumentSimilarity(
							distance, queryInstanceName, instanceName,
							indexFile.getName()));
				}
		...
		{
			Collection<DocumentSimilarity> similarityCollection = similarities
					.get(queryDocument);
			...
			{
				System.out.println(String.format("%-40.40s % 15.5f %-33.33s",
						documentSimilarity.getTargetDocument(),
						documentSimilarity.getDistance(),
						documentSimilarity.getIndex()));
			}
		}
	}
	

	private void trimMatchesToSizeK(
			Multimap<String, DocumentSimilarity> similarities)
	{
		for (String queryDocument : queryDocuments)
		{
			...
			Collections.sort(similarityList,
					new Comparator<DocumentSimilarity>() {
						public int compare(DocumentSimilarity s1,
								DocumentSimilarity s2)
						{
			...
			similarities.replaceValues(
					queryDocument,
					similarityList.subList(0,
							Math.min(k, similarityList.size())));
		}
	}
	...
	private String getInstanceName(Instance instance)
	{
		return instance.toString(classAttribute) + "/"
				+ instance.toString(documentAttribute);
	}
	...
	private void setupIndices()
	{
		...
		{
			System.err
					.println("No .arff files found in current directory, or no .arff files specified");
			System.exit(1);
		}
	}
	...
	private void printProgramStatus()
	{
		...
		System.out.println(String.format("Similarity Measure: %s",
				similarityMeasure));
